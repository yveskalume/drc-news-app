...
const keyExtractor = (item: SourceOverview) => item.id;
...
const SourceList: SourceOverviewListComponent = (props: SourceOverviewListProps) => {
    ...
    const renderItem = useCallback(({item}: { item: SourceOverview }) => {
        return (
    ...
    return (
        <FlatList
            ...
            ListEmptyComponent={() => (
                <Paragraph>Pas de sources disponibles pour le moment.</Paragraph>
            )}
        />
    );
}
